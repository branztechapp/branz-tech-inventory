...
if menu == "📊 Dashboard":
    ...
    if bc_pos:
        ...
        if not match_pos.empty:
            ...
            p_stok = st.session_state.df_local.loc[df['Barcode'] == bc_pos.strip(), 'Stok'].values[0]
    ...
    with col_cart:
        ...
        if not st.session_state.cart:
            ...
            if st.button("🏁 SELESAIKAN TRANSAKSI", use_container_width=True):
                ...
                st.download_button(label="📥 Download Struk (PDF)", data=pdf_output, file_name=f"Struk_{exact_time.strftime('%Y%m%d_%H%M%S')}.pdf", mime="application/pdf", use_container_width=True)
